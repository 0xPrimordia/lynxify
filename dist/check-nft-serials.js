...
async function main() {
    ...
    try {
        ...
        const response = await fetch(`https://testnet.mirrornode.hedera.com/api/v1/tokens/${process.env.NEXT_PUBLIC_NFT_TOKEN_ID}/nfts?account.id=${process.env.OPERATOR_ID}`);
        if (!response.ok) {
